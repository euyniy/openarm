...
When installing OpenArm Cell, please ensure that the available electrical capacity and floor load capacity at the installation site are sufficient for the number of units to be deployed.
In addition, verify in advance that all access routes from the building entrance to the installation location—including door widths and heights, as well as elevator dimensions—provide adequate clearance for physical transport and installation.
